...
ridge_code_bym <- nimbleCode({ 
  ...
  # Defining likelihood
  for(i in 1:N) { 
    log(mu[i]) <- beta0 + inprod(X[i, 1:p], beta[1:p]) + log(E[i]) + s[i] + v[i]
    mu[i] <- E[i]*r[i]
    y[i] ~ dpois(mu[i])
    v[i] ~ dnorm(0, sigmab)
  }
...
plot(main ="Sigma2", ridge03_bym_samples[,"sigmaa"], type ="l")

# sigma squared
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Sigma2", ridge03_bym_samples[,"sigmab"], type ="l")
...
for(i in 1:nrow(data.clean)){
  fitted_ridge03_bym[,i] = ridge03_bym_samples[,"beta0"] + 
    ...
    log(data.clean$Expected[i]) + 
    ridge03_bym_samples[, s[i]]+ 
    ridge03_bym_samples[, v[i]] }

# Mean fitted values
fitted_count_ridge03_bym <- exp(apply(fitted_ridge03_bym, 2, mean))
fitted_count_ridge03_bym
