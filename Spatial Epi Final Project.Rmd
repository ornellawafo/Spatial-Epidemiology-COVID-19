...
data %>% 
 dplyr::select(Cases, Pop_density, 
...
#hist(log(data.clean$Pop_density))
data.clean %>% 
 dplyr::select(Cases, Pop_density, 
         Prop_0.24, Prop_25.64, Prop_65, 
          Prop_labor,  Log_postsec,  Prop_female, beds_per_hosp) %>% ## select variables to plot
  gather() %>%                             
  ggplot(aes(value)) +                     
    facet_wrap(~key, scales = "free") +    
    geom_histogram(bins = 5) +
...
Inits_usual <- list(beta0 = 0,
                    ...
                    sigma2 = 1, 
                    #tau2 = 0.1,
                    s = rnorm(length(num.nb)) )# S sampled from N dist (do we sample ours from poisson?)
...
# (7) Run the model
usual01_runMCMC <- runMCMC(usual01_CMCMC, niter = 500000, nburnin = 15000, thin = 20, nchains = 2, WAIC = T)
...
par(mfrow=c(3,3), mar=c(3, 3, 3, 2) + 0.1)
...
for(i in 1:nrow(data.clean)){
  fitted_usual01[,i] = usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(usual01_samples[,1:14]) +
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_usual02[,i] = usual02_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(usual02_samples[,1:14]) +
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_usual03[,i] = usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(usual03_samples[,1:14]) +
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual01[,i] = bym_usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual01_samples[,1:14]) +
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual02[,i] = bym_usual02_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual02_samples[,1:14]) + 
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual03[,i] = bym_usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual03_samples[,1:14])  +
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge01[,i]=ridge01_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge01_samples[,1:14]) + ridge01_samples[,17+i]
}
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge02[,i]=ridge02_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge02_samples[,1:14]) + ridge02_samples[,17+i]
}
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge03[,i]=ridge03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge03_samples[,1:14]) + ridge03_samples[,17+i]
}
