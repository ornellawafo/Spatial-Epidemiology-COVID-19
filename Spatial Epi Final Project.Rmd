...
var.cov <- list()
for(i in 1:(ncol(Covariates)-2)){
  var.cov[[i]] <- c(which(abs(cor(Covariates[3:ncol(Covariates)])[,i]) > 0.5))
}

names(var.cov) <- paste(names(Covariates[3:34])) %>% as.vector()
...
# Plot of COVID-19 rate per 100,000  - creating breaks --------------------------------------------------------------------
...
# Note: Female pop highly collinear with pop of 65+ and post-sec

# Poisson regression
poi.fit <- glm(Cases ~ Pop_density + Prop_labor + Prop_0.24 + Prop_25.64 + Prop_65 + Prop_female + Log_income + Log_postsec + Num_hosp + Num_beds + Num_household + offset(log(Expected)), data = data.clean, family = poisson(link="log"))
summary(poi.fit)
...
library(AER)
library(car)
dispersiontest(poi.fit, alternative = "greater") #based from the test, there is overdispersion (indicates residual risk even after adjusting for the covariates)
...
## Building number of neighbors and adjacency lists
```{r}
...
usual_code <- nimbleCode({ 
  ...
  # Defining our priors
 beta0 ~ dnorm(0, sd = 500)
  
  for(j in 1:p){
    beta[j] ~ dnorm(0, sd = 500)  #Betas for the covariates
  }
  ...
  # Defining likelihood
  for(i in 1:N) { 
    log(mu[i]) <- log(E[i]) + beta0 + inprod(X[i, 1:p], beta[1:p]) + s[i]
    y[i] ~ dpois(mu[i])
  }
})
```
...
Consts_usual_01 = list(N = nrow(data.clean),                  # Number of areas
                       p = ncol(data.clean[5:15]),            # Number of covariates
                       X = as.matrix(data.clean[, c(5:15)]),  # Matrix of covariates
...
Inits_usual <- list(beta0 = 150,
                    beta = rep(0, ncol(data.clean[5:15])), 
                    #sigma2 = 10, 
                    tau2 = 0.05,
...
usual01_conf <- configureMCMC(usual01_model, monitors = c( 'beta0', 'beta', 'tau2', 's'))
...
# (7) Run the model
usual01_runMCMC <- runMCMC(usual01_CMCMC, niter = 200000, nburnin = 10000, thin = 25, nchains = 2, WAIC = T)
...
plot(main = paste(names(data.clean[13])), usual01_samples[,"beta[9]"] , type = "l", ylab = "")
plot(main = paste(names(data.clean[14])), usual01_samples[,"beta[10]"] , type = "l", ylab = "")
plot(main = paste(names(data.clean[15])), usual01_samples[,"beta[11]"] , type = "l", ylab = "")
#plot(main = paste(names(data.clean[16])),usual01_samples[,"beta[12]"] , type = "l", ylab = "")
#plot(main = paste(names(data.clean[17])),usual01_samples[,"beta[13]"] , type = "l", ylab = "")
#plot(main = paste(names(data.clean[18])),usual01_samples[,"beta[14]"] , type = "l", ylab = "")
...
means_s_usual_01 <- apply(usual01_samples[ ,16:(ncol(usual01_samples)-1)],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_usual_01 <- t(apply(usual01_samples[ ,16:(ncol(usual01_samples)-1)],2, function(x) quantile(x, probs=c(0.015,0.975))))
...
mean_beta0_usual01 <- mean(usual01_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_usual_01 <- t(quantile(usual01_samples[ , 15], probs=c(0.015,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_usual01[,i] = usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(usual01_samples[,1:14]) +
                           usual01_samples[, 16+i-1] 

}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_usual01_df <- array(dim= c( ncol(fitted_usual01), 3)) %>% as.data.frame()

# Fitted values
fitted_usual01_df[, 1] = exp(apply(fitted_usual01, 2, mean))/(1+exp(apply(fitted_usual01, 2, mean)))*100

# 95% Cr I LL
fitted_usual01_df[, 2] = exp(apply(fitted_usual01, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_usual01, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_usual01_df[, 3] = exp(apply(fitted_usual01, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_usual01, 2, function(x) quantile(x, probs=0.975))))*100
...
cbind(data.clean$Cases,fitted_usual01_df)
```
...
Consts_usual_02 = list(N = nrow(data.clean),                  # Number of areas
                       p = ncol(data.clean[5:21]),            # Number of covariates
                       X = as.matrix(data.clean[, c(5:21)]),  # Matrix of covariates
...
Inits_usual_02 <- list(beta0 = 0,
                    beta = rep(0, ncol(data.clean[5:21])), 
                    sigma2 = 1, 
...
mean_beta0_usual02 <- mean(usual02_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_usual_02 <- t(quantile(usual02_samples[ , 15], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_usual02[,i] = usual02_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(usual02_samples[,1:14]) +
                           usual02_samples[, 16+i-1] 

}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_usual02_df <- array(dim= c( ncol(fitted_usual02), 3)) %>% as.data.frame()

# Fitted values
fitted_usual02_df[, 1] = exp(apply(fitted_usual02, 2, mean))/(1+exp(apply(fitted_usual02, 2, mean)))*100

# 95% Cr I LL
fitted_usual02_df[, 2] = exp(apply(fitted_usual02, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_usual02, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_usual02_df[, 3] = exp(apply(fitted_usual02, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_usual02, 2, function(x) quantile(x, probs=0.975))))*100
...
#Calculating the Beta 95% CrI 
CrI_betas_usual_03=t(apply(usual03_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the Beta 95% CrI 
CrI_s_usual_03 <- t(apply(usual03_samples[ ,15:(ncol(usual03_samples)-1)],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
mean_beta0_usual03 <- mean(usual03_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_usual_03 <- t(quantile(usual03_samples[ , 15], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_usual03[,i] = usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(usual03_samples[,1:14]) +
                           usual03_samples[, 16+i-1] 

}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_usual03_df <- array(dim= c( ncol(fitted_usual03), 3)) %>% as.data.frame()

# Fitted values
fitted_usual03_df[, 1] = exp(apply(fitted_usual03, 2, mean))/(1+exp(apply(fitted_usual03, 2, mean)))*100

# 95% Cr I LL
fitted_usual03_df[, 2] = exp(apply(fitted_usual03, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_usual03, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_usual03_df[, 3] = exp(apply(fitted_usual03, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_usual03, 2, function(x) quantile(x, probs=0.975))))*100

# Renaming columns
colnames(fitted_usual03_df) <- c("fitted_mean_usual03", "fitted_lower_usual03" , "fitted_upper_usual03")

#Top 6 rows
head(fitted_usual03_df)

#Predicted vs observed cases
cbind(data.clean$Cases,fitted_usual03_df)
...
# (2) Define the compiled nimble model
bym_usual01_Cmodel <- compileNimble(bym_usual01_model,
                              showCompilerOutput = TRUE)      
...
# Betas (Covariates) - log scale
par(mfrow=c(4,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main = paste(names(data.clean[5])),  bym_usual01_samples[,"beta[1]"] , type = "l") # Why are all beta's = 1
...
plot(main = paste(names(data.clean[13])), bym_usual01_samples[,"beta[9]"] , type = "l")
plot(main = paste(names(data.clean[14])),bym_usual01_samples[,"beta[10]"] , type = "l")
plot(main = paste(names(data.clean[15])),bym_usual01_samples[,"beta[11]"] , type = "l")
plot(main = paste(names(data.clean[16])),bym_usual01_samples[,"beta[12]"] , type = "l")
plot(main = paste(names(data.clean[17])),bym_usual01_samples[,"beta[13]"] , type = "l")
plot(main = paste(names(data.clean[18])),bym_usual01_samples[,"beta[14]"] , type = "l")
...
plot(main = paste(names(data.clean[13])),density(bym_usual01_samples[,"beta[9]"]))
plot(main = paste(names(data.clean[14])),density(bym_usual01_samples[,"beta[10]"]))
plot(main = paste(names(data.clean[15])),density(bym_usual01_samples[,"beta[11]"]))
plot(main = paste(names(data.clean[16])),density(bym_usual01_samples[,"beta[12]"]))
plot(main = paste(names(data.clean[17])),density(bym_usual01_samples[,"beta[13]"]))
plot(main = paste(names(data.clean[18])),density(bym_usual01_samples[,"beta[14]"]))
...
#Betas
#Calculating the Beta means
means_betas_bym_usual01 <- apply(bym_usual01_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_bym_usual01 <- t(apply(bym_usual01_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.015,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_bym_usual01 <- t(matrix(c(means_betas_bym_usual01, CrI_betas_bym_usual01), ncol=14, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_bym_usual01 <- apply(bym_usual01_samples[ ,15:32],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_bym_usual01 <- t(apply(bym_usual01_samples[ ,15:32],2, function(x) quantile(x, probs=c(0.015,0.975))))
...
#Calculating the S means
means_v_bym_usual01 <- apply(bym_usual01_samples[ ,34:ncol(bym_usual01_samples)],2, mean)

#Calculating the Beta 95% CrI 
CrI_v_bym_usual01 <- t(apply(bym_usual01_samples[ ,34:ncol(bym_usual01_samples)],2, function(x) quantile(x, probs=c(0.015,0.975))))
...
# Converting means and CrIs from log to regular scale
exp(mean_CrI_v_bym_usual01)



################## beta 0 

mean_beta0_bym_usual01 <- mean(bym_usual01_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_bym_usual01 <- t(quantile(bym_usual01_samples[ , 15], probs=c(0.015,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual01[,i] = bym_usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(bym_usual01_samples[,1:14]) +
                           bym_usual01_samples[, 16+i-1] + bym_usual01_samples[ , 35+i-1]

}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual01_df <- array(dim= c( ncol(fitted_bym_usual01), 3)) %>% as.data.frame()

# Fitted values
fitted_bym_usual01_df[, 1] = exp(apply(fitted_bym_usual01, 2, mean))/(1+exp(apply(fitted_bym_usual01, 2, mean)))*100

# 95% Cr I LL
fitted_bym_usual01_df[, 2] = exp(apply(fitted_bym_usual01, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_bym_usual01, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_bym_usual01_df[, 3] = exp(apply(fitted_bym_usual01, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_bym_usual01, 2, function(x) quantile(x, probs=0.975))))*100
...
# (2) Define the compiled nimble model
bym_usual02_Cmodel <- compileNimble(bym_usual02_model,
                              showCompilerOutput = TRUE)      
...
# Betas (Covariates) - log scale
par(mfrow=c(4,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main = paste(names(data.clean[5])),  bym_usual02_samples[,"beta[1]"] , type = "l") # Why are all beta's = 1
...
plot(main = paste(names(data.clean[13])), bym_usual02_samples[,"beta[9]"] , type = "l")
plot(main = paste(names(data.clean[14])),bym_usual02_samples[,"beta[10]"] , type = "l")
plot(main = paste(names(data.clean[15])),bym_usual02_samples[,"beta[11]"] , type = "l")
plot(main = paste(names(data.clean[16])),bym_usual02_samples[,"beta[12]"] , type = "l")
plot(main = paste(names(data.clean[17])),bym_usual02_samples[,"beta[13]"] , type = "l")
plot(main = paste(names(data.clean[18])),bym_usual02_samples[,"beta[14]"] , type = "l")
...
plot(main = paste(names(data.clean[13])),density(bym_usual02_samples[,"beta[9]"]))
plot(main = paste(names(data.clean[14])),density(bym_usual02_samples[,"beta[10]"]))
plot(main = paste(names(data.clean[15])),density(bym_usual02_samples[,"beta[11]"]))
plot(main = paste(names(data.clean[16])),density(bym_usual02_samples[,"beta[12]"]))
plot(main = paste(names(data.clean[17])),density(bym_usual02_samples[,"beta[13]"]))
plot(main = paste(names(data.clean[18])),density(bym_usual02_samples[,"beta[14]"]))
...
#Betas
#Calculating the Beta means
means_betas_bym_usual02 <- apply(bym_usual02_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_bym_usual02 <- t(apply(bym_usual02_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_bym_usual02 <- t(matrix(c(means_betas_bym_usual02, CrI_betas_bym_usual02), ncol=14, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_bym_usual02 <- apply(bym_usual02_samples[ ,15:32],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_bym_usual02 <- t(apply(bym_usual02_samples[ ,15:32],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the S means
means_v_bym_usual02 <- apply(bym_usual02_samples[ ,34:ncol(bym_usual02_samples)],2, mean)

#Calculating the Beta 95% CrI 
CrI_v_bym_usual02 <- t(apply(bym_usual02_samples[ ,34:ncol(bym_usual02_samples)],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
mean_beta0_bym_usual02 <- mean(bym_usual02_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_bym_usual02 <- t(quantile(bym_usual02_samples[ , 15], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual02[,i] = bym_usual02_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(bym_usual02_samples[,1:14]) + 
                           bym_usual02_samples[, 16+i-1] + bym_usual02_samples[ , 35+i-1]
}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual02_df <- array(dim= c( ncol(fitted_bym_usual02), 3)) %>% as.data.frame()

# Fitted values
fitted_bym_usual02_df[, 1] = exp(apply(fitted_bym_usual02, 2, mean))/(1+exp(apply(fitted_bym_usual02, 2, mean)))*100

# 95% Cr I LL
fitted_bym_usual02_df[, 2] = exp(apply(fitted_bym_usual02, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_bym_usual02, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_bym_usual02_df[, 3] = exp(apply(fitted_bym_usual02, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_bym_usual02, 2, function(x) quantile(x, probs=0.975))))*100
...
# (2) Define the compiled nimble model
bym_usual03_Cmodel <- compileNimble(bym_usual03_model,
                              showCompilerOutput = TRUE)      
...
# Betas (Covariates) - log scale
par(mfrow=c(4,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main = paste(names(data.clean[5])),  bym_usual03_samples[,"beta[1]"] , type = "l") # Why are all beta's = 1
...
plot(main = paste(names(data.clean[13])), bym_usual03_samples[,"beta[9]"] , type = "l")
plot(main = paste(names(data.clean[14])),bym_usual03_samples[,"beta[10]"] , type = "l")
plot(main = paste(names(data.clean[15])),bym_usual03_samples[,"beta[11]"] , type = "l")
plot(main = paste(names(data.clean[16])),bym_usual03_samples[,"beta[12]"] , type = "l")
plot(main = paste(names(data.clean[17])),bym_usual03_samples[,"beta[13]"] , type = "l")
plot(main = paste(names(data.clean[18])),bym_usual03_samples[,"beta[14]"] , type = "l")
...
plot(main = paste(names(data.clean[13])),density(bym_usual03_samples[,"beta[9]"]))
plot(main = paste(names(data.clean[14])),density(bym_usual03_samples[,"beta[10]"]))
plot(main = paste(names(data.clean[15])),density(bym_usual03_samples[,"beta[11]"]))
plot(main = paste(names(data.clean[16])),density(bym_usual03_samples[,"beta[12]"]))
plot(main = paste(names(data.clean[17])),density(bym_usual03_samples[,"beta[13]"]))
plot(main = paste(names(data.clean[18])),density(bym_usual03_samples[,"beta[14]"]))
...
#Betas
#Calculating the Beta means
means_betas_bym_usual03 <- apply(bym_usual03_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_bym_usual03 <- t(apply(bym_usual03_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_bym_usual03 <- t(matrix(c(means_betas_bym_usual03, CrI_betas_bym_usual03), ncol=14, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_bym_usual03 <- apply(bym_usual03_samples[ ,15:32],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_bym_usual03 <- t(apply(bym_usual03_samples[ ,15:32],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the S means
means_v_bym_usual03 <- apply(bym_usual03_samples[ ,34:ncol(bym_usual03_samples)],2, mean)

#Calculating the Beta 95% CrI 
CrI_v_bym_usual03 <- t(apply(bym_usual03_samples[ ,34:ncol(bym_usual03_samples)],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
mean_beta0_bym_usual03 <- mean(bym_usual03_samples[ ,15])

#Calculating the Beta 95% CrI 
CrI_beta0_bym_usual03 <- t(quantile(bym_usual03_samples[ , 15], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_bym_usual03[,i] = bym_usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(bym_usual03_samples[,1:14])  +
                           bym_usual03_samples[, 16+i-1] + bym_usual03_samples[ , 35+i-1]
}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual03_df <- array(dim= c( ncol(fitted_bym_usual03), 3)) %>% as.data.frame()

# Fitted values
fitted_bym_usual03_df[, 1] = exp(apply(fitted_bym_usual03, 2, mean))/(1+exp(apply(fitted_bym_usual03, 2, mean)))*100

# 95% Cr I LL
fitted_bym_usual03_df[, 2] = exp(apply(fitted_bym_usual03, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_bym_usual03, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_bym_usual03_df[, 3] = exp(apply(fitted_bym_usual03, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_bym_usual03, 2, function(x) quantile(x, probs=0.975))))*100
...
# (3) Define initial values for the MCMC chain
Inits_ridge <- list(beta0=500, beta=rep(0, ncol(data.clean[5:15])),
...
#Betas
#Calculating the Beta means
means_betas_ridge01 <- apply(ridge01_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge01 <- t(apply(ridge01_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the S means
means_s_ridge01 <- apply(ridge01_samples[ ,17:34],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge01 <- t(apply(ridge01_samples[ ,17:34],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
means_sigma_ridge01 <- mean(ridge01_samples[ ,35])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge01 <- t(quantile(ridge01_samples[ ,35], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge01[,i]=ridge01_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(ridge01_samples[,1:14]) + ridge01_samples[,17+i]
}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge01_df <- array(dim= c( ncol(fitted_ridge01), 3)) %>% as.data.frame()

# Fitted values
fitted_ridge01_df[, 1] = exp(apply(fitted_ridge01, 2, mean))/(1+exp(apply(fitted_ridge01, 2, mean)))*100

# 95% Cr I LL
fitted_ridge01_df[, 2] = exp(apply(fitted_ridge01, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_ridge01, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_ridge01_df[, 3] = exp(apply(fitted_ridge01, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_ridge01, 2, function(x) quantile(x, probs=0.975))))*100
...
# (3) Define initial values for the MCMC chain
Inits_ridge <- list(beta0=1, beta=rep(0, ncol(data.clean[5:21])),
...
#Betas
#Calculating the Beta means
means_betas_ridge02 <- apply(ridge02_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge02 <- t(apply(ridge02_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the S means
means_s_ridge02 <- apply(ridge02_samples[ ,17:34],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge02 <- t(apply(ridge02_samples[ ,17:34],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge02 <- mean(ridge02_samples[ ,35])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge02 <- t(quantile(ridge02_samples[ ,35], probs=c(0.025,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge02[,i]=ridge02_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(ridge02_samples[,1:14]) + ridge02_samples[,17+i]
}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge02_df <- array(dim= c( ncol(fitted_ridge02), 3)) %>% as.data.frame()

# Fitted values
fitted_ridge02_df[, 1] = exp(apply(fitted_ridge02, 2, mean))/(1+exp(apply(fitted_ridge02, 2, mean)))*100

# 95% Cr I LL
fitted_ridge02_df[, 2] = exp(apply(fitted_ridge02, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_ridge02, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_ridge02_df[, 3] = exp(apply(fitted_ridge02, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_ridge02, 2, function(x) quantile(x, probs=0.975))))*100
...
#Betas
#Calculating the Beta means
means_betas_ridge03 <- apply(ridge03_samples[,1:14],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge03 <- t(apply(ridge03_samples[ ,1:14],2, function(x) quantile(x, probs=c(0.035,0.975))))
...
#Calculating the S means
means_s_ridge03 <- apply(ridge03_samples[ ,17:34],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge03 <- t(apply(ridge03_samples[ ,17:34],2, function(x) quantile(x, probs=c(0.035,0.975))))
...
#Calculating the sigma means
means_sigma_ridge03 <- mean(ridge03_samples[ ,35])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge03 <- t(quantile(ridge03_samples[ ,35], probs=c(0.035,0.975)))
...
#Loop to generate fitted values
for(i in 1:nrow(data.clean)){
  fitted_ridge03[,i]=ridge03_samples[,"beta0"] + as.matrix(data.clean[i,5:21]) %*% t(ridge03_samples[,1:14]) + ridge03_samples[,17+i]
}

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge03_df <- array(dim= c( ncol(fitted_ridge03), 3)) %>% as.data.frame()

# Fitted values
fitted_ridge03_df[, 1] = exp(apply(fitted_ridge03, 2, mean))/(1+exp(apply(fitted_ridge03, 2, mean)))*100

# 95% Cr I LL
fitted_ridge03_df[, 2] = exp(apply(fitted_ridge03, 2, function(x) quantile(x, probs=0.025)))/(1+exp(apply(fitted_ridge03, 2, function(x) quantile(x, probs=0.025))))*100

# 95% Cr I UL
fitted_ridge03_df[, 3] = exp(apply(fitted_ridge03, 2, function(x) quantile(x, probs=0.975)))/(1+exp(apply(fitted_ridge03, 2, function(x) quantile(x, probs=0.975))))*100
