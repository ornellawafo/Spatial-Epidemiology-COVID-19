...
# 5- Converting means and CrIs from log to regular scale
exp(mean_CrI_betas_ridge01)

################### Spatial effect per region

# 1- Calculating the S means
means_s_ridge01 <- apply(ridge01_samples[ ,11:28],2, mean)

# 2- Calculating the S 95% CrI 
...
# 1- Calculating the sigma means
means_sigma_ridge01 <- mean(ridge01_samples[ ,29])

# 2- Calculating the sigma 95% CrI 
...
# 1- Calculating the lambda means
mean_lambda_ridge01 <- mean(ridge01_samples[ ,"lambda"])

# 2- Calculating the Beta 95% CrI 
CrI_lambda_ridge01 <- t(quantile(ridge01_samples[ , "lambda"], probs=c(0.025,0.975)))

# 3- Combining the means and CIs into a dataframe
mean_CrI_lambda_ridge01 <- cbind(mean_lambda_ridge01, CrI_lambda_ridge01) %>% as.data.frame()

# 4- Renaming the columns
colnames(mean_CrI_lambda_ridge01) <- c("Mean", "95% CrI LL", "95% CrI UL")

# 5- Converting means and CrIs from log to regular scale
exp(mean_CrI_lambda_ridge01)
