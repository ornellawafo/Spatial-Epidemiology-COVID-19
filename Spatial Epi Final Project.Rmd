...
# (3) Configure the MCMC chain
bym_usual01_conf <- configureMCMC(bym_usual01_model, monitors = c('beta', 'sigma2', 's', 'v', 'h'))
...
#Combining the means and CIs into a dataframe
mean_CrI_betas_ubym_sual_01 <- t(matrix(c(means_betas_bym_usual_01, CrI_betas_bym_usual_01), ncol=14, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_bym_usual_01 <- apply(bym_usual01_samples[ ,15:(ncol(bym_usual01_samples)-1)],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_bym_usual_01 <- t(apply(bym_usual01_samples[ ,15:(ncol(bym_usual01_samples)-1)],2, function(x) quantile(x, probs=c(0.015,0.975))))
