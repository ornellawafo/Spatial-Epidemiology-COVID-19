...
coef.uni <- round(c(poi.fit1$coefficient[2], poi.fit2$coefficient[2], poi.fit3$coefficient[2],
                    poi.fit3$coefficient[3], poi.fit3$coefficient[4], poi.fit4$coefficient[2],
                    poi.fit5$coefficient[2], poi.fit6$coefficient[2]))
coef.uni.se <- c(0.047992, 5.394, 136.9, 136.9, 136.9, 2.353, 0.0002422, -0.16196)
coef.multi <- round(c(poi.fit7$coefficient[2], poi.fit7$coefficient[3], poi.fit7$coefficient[4],
                      poi.fit7$coefficient[5], poi.fit7$coefficient[6], poi.fit7$coefficient[7],
                      poi.fit7$coefficient[8], poi.fit7$coefficient[9]))
...
**Table 1.** Comparison of parameter estimates of simple and multiple linear regressions

| Variable | Univariate estimate, $\hat{\beta}$ (SE) | Multivariate estimate, $\hat{\beta}$ (SE) |
|:-------- | :---------------------- | :---------------------- |
| st_pop_density | `r round(poi.fit1$coefficient[2], 2)` (0.047992) | `r round(Q3d.regression$coefficient[2], 2)` [-1.78, -0.49] |
| Prop_female  | `r round(poi.fit2$coefficient[2], 2)` [-1.60, 1.55] | `r round(Q3d.regression$coefficient[3], 2)` [1.56, 2.86] |
| Prop_0.24 | `r round(poi.fit3$coefficient[2], 2)` [6.29, 7.93] | `r round(Q3d.regression$coefficient[4], 2)` [-0.19, 1.95] |
| Prop_25.64 | `r round(poi.fit3$coefficient[2], 2)` [2.71, 3.23] | `r round(Q3d.regression$coefficient[5], 2)` [2.62, 3.62] |
| Prop_65 | `r round(poi.fit3$coefficient[2], 2)`
| Prop_labor | `r round(poi.fit4$coefficient[2], 2)`
| beds_per_hosp | `r round(poi.fit5$coefficient[2], 2)`
| household_poverty_rate | `r round(poi.fit6$coefficient[2], 2)`
...
```{r Nimble: iCAR Usual fixed priors model}
...
```{r iCAR Adjacency  MCMC chains, echo = F}
# beta0
...
```{r iCAR Adjacency  Posterior density curves, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
```{r iCAR K3  Posterior density curves, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
```{r iCAR K5  Posterior density curves, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
```{r Nimble: Usual fixed priors model}
# Defining the usual fixed priors model ------------------------------------------------------
...
for(i in 1:nrow(data.clean)){
  fitted_bym_usual01[,i] = bym_usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% 
    t(bym_usual01_samples[,1:8]) 
  + bym_usual01_samples[, 10+i-1] 
  + bym_usual01_samples[ , 29+i-1]
}
...
# Mean fitted Cases
fitted_bym_usual_01 <- fitted_SIR_bym_usual01*data.clean$Expected

# 95% CrI Cases
fitted_bym_usual01_CrI <-  data.clean$Expected *fitted_bym_usual01_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual01_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_bym_usual01,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_bym_usual_01), 
...
```{r BYM Adjacency MCMC chains, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(5, 4, 4, 2) + 0.1)
...
```{r BYM Adjacency Posterior density curve, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_bym_usual02[,i] = bym_usual02_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual02_samples [, 1:8]) 
  + bym_usual02_samples[ , 10+i-1] 
  + bym_usual02_samples[ , 29+i-1]
}
# Fitted values - distribution
fitted_bym_usual02_dist <- data.clean$Expected * exp(fitted_bym_usual02)
...
# Mean fitted Cases
fitted_bym_usual_02 <- fitted_SIR_bym_usual02*data.clean$Expected

# 95% CrI Cases
fitted_bym_usual02_CrI <-  data.clean$Expected *fitted_bym_usual02_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual02_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_bym_usual02,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_bym_usual_02), 
...
```{r BYM K3 Posterior density curves, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_bym_usual03[,i] = bym_usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% 
    t(bym_usual03_samples[,1:8])
  + bym_usual03_samples[, 10+i-1] 
  + bym_usual03_samples[ , 29+i-1]
}
...
# Mean fitted Cases
fitted_bym_usual_03 <- fitted_SIR_bym_usual03*data.clean$Expected

# 95% CrI Cases
fitted_bym_usual03_CrI <-  data.clean$Expected *fitted_bym_usual03_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual03_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_bym_usual03,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_bym_usual_03), 
...
```{r BYM K5 Posterior density curves, echo = F}
# beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
ridge_code <- nimbleCode({ 
  ...
  # Defining likelihood
  for(i in 1:N) { 
    log(r[i]) <- beta0 + inprod(X[i, 1:p], beta[1:p]) + log(E[i]) + s[i]
    mu[i] <- E[i]*r[i]
    y[i] ~ dpois(mu[i])
  }
})
...
Inits_ridge <- list(beta0=0, beta=rep(0, ncol(data.clean[5:ncol(data.clean)])),
                    sigma=2, lambda=1, s=rnorm(nrow(data.clean)))
...
# (7) Run the model
ridge01_runMCMC=runMCMC(ridge01_CMCMC, nburnin=10000, niter=500000, nchains=2, thin=25, WAIC=T)
...
```{r Ridge Adjacency Summaries}
# (10) Posterior summaries: Means and CrI  --------------------------------------------------------

#Calculating the Beta means (beta0 included, last variable)
means_betas_ridge01 <- apply(ridge01_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge01 <- t(apply(ridge01_samples[ ,1:9],2, function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge01 <- t(matrix(c(means_betas_ridge01, CrI_betas_ridge01), ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
################### Spatial effect per region

#Calculating the S means
means_s_ridge01 <- apply(ridge01_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge01 <- t(apply(ridge01_samples[ ,11:28],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
means_sigma_ridge01 <- mean(ridge01_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge01 <- t(quantile(ridge01_samples[ ,29], probs=c(0.025,0.975)))
...
for(i in 1:nrow(data.clean)){
  fitted_ridge01[,i] = ridge01_samples[,"beta0"] + 
    as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge01_samples[,1:8]) + 
    ridge01_samples[, 11+i-1] }

# Fitted values - distribution
fitted_ridge_01_dist <- data.clean$Expected * exp(fitted_ridge01)

# Mean fitted SIR
fitted_SIR_ridge01 <- exp(apply(fitted_ridge01, 2, mean))

# Mean fitted Cases
fitted_ridge_01 <- fitted_SIR_ridge01*data.clean$Expected

# 95% CrI SIR
fitted_ridge_01_CrI_SIR <- t(exp(apply(fitted_ridge01, 2, function(x) quantile(x, probs=c(0.025, 0.975)))))

# 95% CrI Cases
fitted_ridge_01_CrI <-  data.clean$Expected *fitted_ridge_01_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge01_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_ridge01,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_ridge_01), 
                                "CrI LL"    =  round(fitted_ridge_01_CrI[,1]),
                                "CrI UL"    =  round(fitted_ridge_01_CrI[,2]))

# Top 6 rows
head(fitted_ridge01_df)
```

```{r Ridge Adjacency MCMC chains, echo = F}
#Beta0
plot(main ="Intercept", ridge01_samples[,"beta0"], type = "l")

# Betas (Covariates) - log scale
par(mfrow=c(2,4), mar=c(2,4.5,2.1,1)+0.1)
...
```{r Ridge Adjacency Posterior density curve, echo = F}
# Betas (Covariates) - log scale
...
# sigma squared
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Sigma Squared", density(ridge01_samples[,"sigma"]))
...
# (1) Define ridge nimble model
ridge02_model=nimbleModel(code=ridge_code, 
                          constants=Consts_ridge_02, data=outcome, inits=Inits_ridge)
...
# (7) Run the model
ridge02_runMCMC=runMCMC(ridge02_CMCMC, nburnin=10000, niter=500000, nchains=2, thin=25, WAIC=T)
...
```{r Ridge K3 Summaries}
# (10) Posterior summaries: Means and CrI  --------------------------------------------------------

#Betas
#Calculating the Beta means
means_betas_ridge02 <- apply(ridge02_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge02 <- t(apply(ridge02_samples[ ,1:9],2, 
                             function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge02 <- t(matrix(c(means_betas_ridge02, CrI_betas_ridge02), 
                                   ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_ridge02 <- apply(ridge02_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge02 <- t(apply(ridge02_samples[ ,11:28],2, 
                         function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge02 <- mean(ridge02_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge02 <- t(quantile(ridge02_samples[ ,29], probs=c(0.025,0.975)))
...
for(i in 1:nrow(data.clean)){
  fitted_ridge02[,i] = ridge02_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge02_samples[,1:8])
                      + ridge02_samples[, 11+i-1] }

# Mean fitted SIR
fitted_SIR_ridge02 <- exp(apply(fitted_ridge02, 2, mean))

# Mean fitted Cases
fitted_ridge_02 <- fitted_SIR_ridge02*data.clean$Expected

# 95% CrI SIR
fitted_ridge_02_CrI_SIR <- t(exp(apply(fitted_ridge02, 2, 
                                       function(x) quantile(x, probs=c(0.025, 0.975)))))

# Creating Dataframe to store fitted values and quantiles in regular svale
# fitted_ridge02_df <- cbind(fitted_SIR_ridge02, fitted_car_ridge_02_CrI)

# 95% CrI Cases
fitted_ridge_02_CrI <-  data.clean$Expected *fitted_ridge_02_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge02_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_ridge02,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_ridge_02), 
                                "CrI LL"    =  round(fitted_ridge_02_CrI[,1]),
                                "CrI UL"    =  round(fitted_ridge_02_CrI[,2]))

# Top 6 rows
head(fitted_ridge02_df)
```

```{r Ridge K3 MCMC chains, echo = F}
# Beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
...
```{r Ridge K3 Posterior density curve, echo = F}
# Betas (Covariates) - log scale
par(mfrow=c(3,4), mar=c(2,4.5,2.1,1)+0.1)
...
# Sigma squared
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Sigma Squared", density(ridge02_samples[,"sigma"]))
...
# (7) Run the model
ridge03_runMCMC=runMCMC(ridge03_CMCMC, nburnin=10000, niter=200000, nchains=2, thin=25, WAIC=T)
...
```{r Ridge K5 Summaries}
# (10) Posterior summaries: Means and CrI  -------------------------------------------------------

#Calculating the Beta means
means_betas_ridge03 <- apply(ridge03_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge03 <- t(apply(ridge03_samples[ ,1:9],2, 
                             function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge03 <- t(matrix(c(means_betas_ridge03, CrI_betas_ridge03), 
                                   ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
################### Spatial effects per region

#Calculating the S means
means_s_ridge03 <- apply(ridge03_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge03 <- t(apply(ridge03_samples[ ,11:28],2, 
                         function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge03 <- mean(ridge03_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge03 <- t(quantile(ridge03_samples[ ,29], probs=c(0.025,0.975)))
...
for(i in 1:nrow(data.clean)){
  fitted_ridge03[,i] = ridge03_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge03_samples[,1:8])
...
# Mean fitted Cases
fitted_ridge_03 <- fitted_SIR_ridge03*data.clean$Expected

# 95% CrI SIR
fitted_ridge_03_CrI_SIR <- t(exp(apply(fitted_ridge03, 2, 
                                       function(x) quantile(x, probs=c(0.025, 0.975)))))

# Creating Dataframe to store fitted values and quantiles in regular svale
# fitted_ridge03_df <- cbind(fitted_car_ridge_03_mean, fitted_car_ridge_03_CrI)

# 95% CrI Cases
fitted_ridge_03_CrI <-  data.clean$Expected *fitted_ridge_03_CrI_SIR

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge03_df <- data.frame("Observed SIR"   =  round(data.clean$SIR, 2), 
                                "Fitted SIR"     =  round(fitted_SIR_ridge03,2), 
                                "Observed Cases" =  round(data.clean$Cases), 
                                "Fitted Cases" =  round(fitted_ridge_03), 
                                "CrI LL"    =  round(fitted_ridge_03_CrI[,1]),
                                "CrI UL"    =  round(fitted_ridge_03_CrI[,2]))

# Top 6 rows
head(fitted_ridge03_df)
```

```{r Ridge K5 MCMC chains, echo = F}
# Beta0
...
```{r Ridge K5 Posterior density curves, echo = F}
# Betas (Covariates) - log scale
par(mfrow=c(3,4), mar=c(2,4.5,2.1,1)+0.1)
...
#sigma squared
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Sigma Squared", density(ridge03_samples[,"sigma"]))
...
# Defining model -----------------------------------------------------------------------------
# (1) Define ridge nimble model
ridge01_model = nimbleModel(code=ridge_code_bym, constants=Consts_ridge_01, data=outcome,
                            inits=Inits_ridge)
...
```{r Ridge BYM Adjacency Summaries}
# (10) Posterior summaries: Means and CrI  --------------------------------------------------------

#Calculating the Beta means (beta0 included, last variable)
means_betas_ridge01_bym <- apply(ridge01_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge01_bym <- t(apply(ridge01_samples[ ,1:9],2, function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge01_bym <- t(matrix(c(means_betas_ridge01_bym, CrI_betas_ridge01_bym), ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_ridge01_bym <- apply(ridge01_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge01_bym <- t(apply(ridge01_samples[ ,11:28],2, 
                             function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge01_bym <- mean(ridge01_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge01_bym <- t(quantile(ridge01_samples[ ,29], probs=c(0.025,0.975)))
...
mean_beta0_ridge01_bym <- mean(ridge01_samples[ ,"lambda"])

#Calculating the Beta 95% CrI 
CrI_beta0_ridge01_bym <- t(quantile(ridge01_samples[ , "lambda"], probs=c(0.025,0.975)))
...
#Creating matrix to store fitted values
fitted_ridge01_bym = matrix(NA, nrow=nrow(ridge01_samples), ncol= nrow(data.clean))

#Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_ridge01_bym[,i] = ridge01_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge01_samples[,1:8])
                      + log(data.clean$Expected[i]) +
                      + ridge01_samples[, 11+i-1] }

# Fitted values - distribution
fitted_bym_ridge_01_dist <- exp(fitted_ridge01)

# Mean fitted values
fitted_bym_ridge_01_mean <- apply(fitted_bym_ridge_01_dist, 2, mean)

# 95% CrI
fitted_bym_ridge_01_CrI <- t(apply(fitted_bym_ridge_01_dist, 2, function(x) quantile(x, probs=c(0.025, 0.975))))

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge01_df_bym <- cbind(fitted_bym_ridge_01_mean, fitted_bym_ridge_01_CrI)

# Renaming columns
colnames(fitted_ridge01_df_bym) <- c("fitted_mean_ridge01", "fitted_lower_ridge01" , "fitted_upper_ridge01")

# Top 6 rows
head(fitted_ridge01_df_bym)

# Predicted vs observed cases
cbind(data.clean$Cases, fitted_ridge01_df_bym) #similar fitted values
```

```{r Ridge BYM Adjacency MCMC chains, echo = F}
# Beta0
plot(main ="Intercept", ridge01_samples[,"beta0"], type = "l")
...
```{r Ridge BYM Adjacency Posterior density curves, echo = F}
# Betas (Covariates) - log scale
par(mfrow=c(3,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Intercept", density(ridge01_samples[,"beta0"]))
plot(main = paste(names(data.clean[5])),density(ridge01_samples[,"beta[1]"])) 
...
# (3) Define initial values for the MCMC chain
Inits_ridge <- list(beta0=0, beta=rep(0, ncol(data.clean[5:ncol(data.clean)])),
                    sigma=2, lambda=50, s=rnorm(nrow(data.clean)))


# Defining model ------------------------------------------------------------------------------

# (1) Define ridge nimble model
ridge02_model=nimbleModel(code=ridge_code_bym, 
                          constants=Consts_ridge_02, data=outcome, inits=Inits_ridge)
...
```{r Ridge BYM K3 Summaries}
# (10) Posterior summaries: Means and CrI  --------------------------------------------------------

#Betas
#Calculating the Beta means
means_betas_ridge02_bym <- apply(ridge02_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge02_bym <- t(apply(ridge02_samples[ ,1:9],2, 
                             function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge02_bym <- t(matrix(c(means_betas_ridge02_bym, CrI_betas_ridge02_bym), 
                                   ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
#Calculating the S means
means_s_ridge02_bym <- apply(ridge02_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge02_bym <- t(apply(ridge02_samples[ ,11:28],2, 
                         function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge02_bym <- mean(ridge02_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge02_bym <- t(quantile(ridge02_samples[ ,29], probs=c(0.025,0.975)))

#Combining the means and CIs into a dataframe
mean_CrI_sigma_ridge02_bym <- cbind(means_sigma_ridge02_bym, CrI_sigma_ridge02_bym) %>% as.data.frame()
...
mean_beta0_ridge02_bym <- mean(ridge02_samples[ ,"lambda"])

#Calculating the Beta 95% CrI 
CrI_beta0_ridge02_bym <- t(quantile(ridge02_samples[ , "lambda"], probs=c(0.025,0.975)))
...
#Creating matrix to store fitted values
fitted_ridge02_bym = matrix(NA, nrow=nrow(ridge02_samples), ncol= nrow(data.clean))

#Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_ridge02_bym[,i] = ridge02_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge02_samples[,1:8])
                      + log(data.clean$Expected[i])
                      + ridge02_samples[, 11+i-1] }

# Fitted values - distribution
fitted_bym_ridge_02_dist <- exp(fitted_ridge02_bym)

# Mean fitted values
fitted_bym_ridge_02_mean <- apply(fitted_bym_ridge_02_dist, 2, mean)

# 95% CrI
fitted_bym_ridge_02_CrI <- t(apply(fitted_bym_ridge_02_dist, 2, 
                                   function(x) quantile(x, probs=c(0.025, 0.975))))

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge02_df_bym <- cbind(fitted_bym_ridge_02_mean, fitted_bym_ridge_02_CrI)

# Renaming columns
colnames(fitted_ridge02_df_bym) <- c("fitted_mean_ridge02", "fitted_lower_ridge02" , "fitted_upper_ridge02")

# Top 6 rows
head(fitted_ridge02_df_bym)

# Predicted vs observed cases
cbind(data.clean$Cases,fitted_ridge02_df_bym)
```

```{r Ridge BYM K3 MCMC chains, echo = F}
#  Beta0
par(mfrow=c(1,1), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Intercept", ridge02_samples[,"beta0"], type = "l")
...
```{r Ridge BYM K3 Posterior distributions, echo = F}
# Betas (Covariates) - log scale
par(mfrow=c(3,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Intercept", density(ridge02_samples[,"beta0"]))
plot(main = paste(names(data.clean[5])),density(ridge02_samples[,"beta[1]"])) 
...
# (1) Define ridge nimble model
ridge03_model=nimbleModel(code=ridge_code_bym, constants=Consts_ridge_03, data=outcome, inits=Inits_ridge)
...
# (7) Run the model
ridge03_runMCMC=runMCMC(ridge03_CMCMC, nburnin=10000, niter=500000, nchains=2, thin=25, WAIC=T)
...
```{r Ridge BYM K5 Summaries}
# (10) Posterior summaries: Means and CrI  --------------------------------------------------------

#Betas
#Calculating the Beta means
means_betas_ridge03_bym <- apply(ridge03_samples[,1:9],2, mean)

#Calculating the Beta 95% CrI 
CrI_betas_ridge03_bym <- t(apply(ridge03_samples[ ,1:9],2, 
                             function(x) quantile(x, probs=c(0.025,0.975))))

#Combining the means and CIs into a dataframe
mean_CrI_betas_ridge03_bym <- t(matrix(c(means_betas_ridge03_bym, CrI_betas_ridge03_bym), 
                                   ncol=9, nrow = 3, byrow=T)) %>% as.data.frame()
...
# Converting means and CrIs from log to regular scale
exp(mean_CrI_betas_ridge03_bym)

################### Spatial effects per region

#Calculating the S means
means_s_ridge03_bym <- apply(ridge03_samples[ ,11:28],2, mean)

#Calculating the Beta 95% CrI 
CrI_s_ridge03_bym <- t(apply(ridge03_samples[ ,11:28],2, 
                         function(x) quantile(x, probs=c(0.025,0.975))))
...
#Calculating the sigma means
means_sigma_ridge03_bym <- mean(ridge03_samples[ ,29])

#Calculating the Beta 95% CrI 
CrI_sigma_ridge03_bym <- t(quantile(ridge03_samples[ ,29], probs=c(0.025,0.975)))

#Combining the means and CIs into a dataframe
mean_CrI_sigma_ridge03_bym <- cbind(means_sigma_ridge03_bym, CrI_sigma_ridge03_bym) %>% as.data.frame()

#Renaming the columns
colnames(mean_CrI_sigma_ridge03_bym) <- c("Mean", "95% CrI LL", "95% CrI UL")

# Consigmaerting means and CrIs from log to regular scale
exp(mean_CrI_sigma_ridge03_bym)

################## Lambda

mean_beta0_ridge03_bym <- mean(ridge03_samples[ ,"lambda"])

#Calculating the Beta 95% CrI 
CrI_beta0_ridge03_bym <- t(quantile(ridge03_samples[ , "lambda"], probs=c(0.025,0.975)))
...
# Converting means and CrIs from log to regular scale
exp(mean_CrI_beta0_ridge03_bym)

# (11) Fitted values -----------------------------------------------------------------------------

#Creating matrix to store fitted values
fitted_ridge03_bym = matrix(NA, nrow=nrow(ridge03_samples), ncol= nrow(data.clean))
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_ridge03_bym[,i] = ridge03_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(ridge03_samples[,1:8])
                      + log(data.clean$Expected[i])
                      + ridge03_samples[, 7+i-1] }

# Fitted values - distribution
fitted_car_ridge_03_dist_bym <- exp(fitted_ridge03_bym)

# Mean fitted values
fitted_car_ridge_03_mean_bym <- apply(fitted_car_ridge_03_dist_bym, 2, mean)

# 95% CrI
fitted_bym_ridge_03_CrI <- t(apply(fitted_car_ridge_03_dist_bym, 2, 
                                   function(x) quantile(x, probs=c(0.025, 0.975))))

# Fitted values - distribution
fitted_bym_ridge_03_dist <- data.clean$Expected * exp(fitted_ridge03)
...
# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge03_df_bym <- cbind(fitted_car_ridge_03_mean_bym, fitted_bym_ridge_03_CrI)

# Renaming columns
colnames(fitted_ridge03_df_bym) <- c("fitted_mean_ridge03", "fitted_lower_ridge03" , "fitted_upper_ridge03")

# Top 6 rows
head(fitted_ridge03_df_bym)

# Predicted vs observed cases
cbind(data.clean$Cases,fitted_ridge03_df_bym)
...
```{r Ridge BYM K5 MCMC chains, echo = F}
# Beta0
plot(main ="Intercept", ridge03_samples[,"beta0"], type = "l")
...
```{r Ridge BYM K5 Posterior density curves, echo = F}
# Betas (Covariates) - log scale
par(mfrow=c(3,4), mar=c(2,4.5,2.1,1)+0.1)
plot(main ="Intercept", density(ridge03_samples[,"beta0"]))
plot(main = paste(names(data.clean[5])),density(ridge03_samples[,"beta[1]"])) 
