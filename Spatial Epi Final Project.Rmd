...
for(i in 1:nrow(data.clean)){
  fitted_usual03[,i] = usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% 
    t(usual03_samples[,1:8]) + usual03_samples[, 8+i-1] 
}
...
# Mean fitted values
fitted_usual_03 <- apply(fitted_usual_03_dist, 2, mean)

# 95% CrI
fitted_usual_03_CrI <- t(apply(fitted_usual_03_dist, 2, function(x) quantile(x, probs=c(0.025, 0.975))))

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_usual03_df <- cbind(fitted_usual_03,fitted_usual_03_CrI )
...
# Calculating the S means
means_s_bym_usual01 <- apply(bym_usual01_samples[ ,8:25],2, mean)

# Calculating the Beta 95% CrI 
CrI_s_bym_usual01 <- t(apply(bym_usual01_samples[ ,8:25],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
means_v_bym_usual01 <- apply(bym_usual01_samples[ ,27:ncol(bym_usual01_samples)],2, mean)

# Calculating the V 95% CrI 
CrI_v_bym_usual01 <- t(apply(bym_usual01_samples[ ,27:ncol(bym_usual01_samples)],2, function(x) quantile(x, probs=c(0.025,0.975))))
...
# Calculating the beta0 means
mean_beta0_bym_usual01 <- mean(bym_usual01_samples[ ,7])

# Calculating the Beta 95% CrI 
CrI_beta0_bym_usual01 <- t(quantile(bym_usual01_samples[ , 7], probs=c(0.025,0.975)))
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_bym_usual01[,i] = bym_usual01_samples[,"beta0"] 
                      + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual01_samples[,1:5])
                      + bym_usual01_samples[, 7+i-1] 
                      + bym_usual01_samples[ , 9+i-1]
}

# Fitted values - distribution
fitted_bym_usual_01_dist <- data.clean$Expected * exp(fitted_bym_usual01)

# Mean fitted values
fitted_bym_usual_01 <- apply(fitted_bym_usual_01, 2, mean)

# 95% CrI
fitted_bym_usual_01_CrI <- t(apply(fitted_bym_usual_01, 2, function(x) quantile(x, probs=c(0.025, 0.975))))

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_bym_usual01_df <- cbind(fitted_bym_usual_01,fitted_bym_usual_01_CrI )
...
ridge_code <- nimbleCode({ 
  ...
  # Defining likelihood
  for(i in 1:N) { 
    log(mu[i]) <- beta0 + inprod(X[i, 1:p], beta[1:p]) + log(E[i]) + s[i]
    #mu[i] <- E[i]*r[i]
    y[i] ~ dpois(mu[i])
  }
})
...
# (7) Run the model
ridge01_runMCMC=runMCMC(ridge01_CMCMC, nburnin=10000, niter=200000, nchains=2, thin=25, WAIC=T)
...
# Fitted values - distribution
fitted_car_ridge_01_mean <- exp(fitted_ridge01)

# Mean fitted values
fitted_car_ridge_01_mean <- apply(fitted_car_ridge_01_dist, 2, mean)
...
# 95% CrI
fitted_car_ridge_01_CrI <- t(apply(fitted_car_ridge_01_dist, 2, function(x) quantile(x, probs=c(0.025, 0.975))))

# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge01_df <- cbind(fitted_car_ridge_01_mean, fitted_car_ridge_01_CrI)
...
#Predicted vs observed cases
cbind(data.clean$Cases,fitted_ridge01_df) #similar fitted values
```
...
# Fitted values - distribution
fitted_car_ridge_03_dist <- exp(fitted_ridge02)

# Mean fitted values
fitted_car_ridge_03_mean <- apply(fitted_car_ridge_03_dist, 2, mean)

# 95% CrI
fitted_car_ridge_03_CrI <- t(apply(fitted_car_ridge_03_dist, 2, 
                                   function(x) quantile(x, probs=c(0.025, 0.975))))
...
# Fitted values - distribution
fitted_car_ridge_03_dist <- exp(fitted_ridge03)
