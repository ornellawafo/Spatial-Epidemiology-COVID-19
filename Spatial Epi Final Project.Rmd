...
```{r iCAR Adjacency-based Model, echo = F, results = F}
...
# 95% CrI Cases
fitted_usual03_CrI <-  data.clean$Expected *fitted_usual_03_CrI_SIR
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_bym_usual01[,i] = bym_usual01_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual01_samples[,1:8]) 
...
# Loop to generate fitted values: log(ri)
for(i in 1:nrow(data.clean)){
  fitted_bym_usual03[,i] = bym_usual03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*% t(bym_usual03_samples[,1:8])
...
# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge02_df <- cbind(fitted_car_ridge_03_mean, fitted_car_ridge_03_CrI)
...
# Mean fitted values
fitted_bym_ridge_01_mean <- apply(fitted_car_ridge_01_dist, 2, mean)

# 95% CrI
fitted_bym_ridge_01_CrI <- t(apply(fitted_car_ridge_01_dist, 2, function(x) quantile(x, probs=c(0.025, 0.975))))
...
#Calculating the Beta 95% CrI 
CrI_beta0_ridge0_bym <- t(quantile(ridge02_samples[ , "lambda"], probs=c(0.025,0.975)))
...
# Fitted values - distribution
fitted_bym_ridge_03_dist <- exp(fitted_ridge02_bym)

# Mean fitted values
fitted_bym_ridge_03_mean <- apply(fitted_bym_ridge_03_dist, 2, mean)

# 95% CrI
fitted_bym_ridge_03_CrI <- t(apply(fitted_bym_ridge_03_dist, 2, 
                                   function(x) quantile(x, probs=c(0.025, 0.975))))


# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge02_df_bym <- cbind(fitted_bym_ridge_03_mean, fitted_bym_ridge_03_CrI)
...
# Creating matrix to store fitted values
fitted_ridge03 = matrix(NA, nrow=nrow(ridge03_samples), ncol= nrow(data.clean))
>>>>>>> df7a8e67c9cc8177a1d823da47977260b87a4e3c
...
# 95% CrI
fitted_car_ridge_03_CrI_bym <- t(apply(fitted_car_ridge_03_dist_bym, 2, 
                                   function(x) quantile(x, probs=c(0.025, 0.975))))
=======
  fitted_ridge03[,i] = ridge03_samples[,"beta0"] + as.matrix(data.clean[i,5:ncol(data.clean)]) %*%t(ridge03_samples[,1:8]) + 
    ridge03_samples[, 10+i-1] 

}

# Fitted values - distribution
fitted_ridge_03_dist <- data.clean$Expected * exp(fitted_ridge03)
...
<<<<<<< HEAD
# Creating Dataframe to store fitted values and quantiles in regular svale
fitted_ridge03_df_bym <- cbind(fitted_car_ridge_03_mean_bym, fitted_car_ridge_03_CrI_bym)
...
=======
# 95% CrI SIR
fitted_ridge_03_CrI_SIR <- t(exp(apply(fitted_ridge03, 2, function(x) quantile(x, probs=c(0.025, 0.975)))))
